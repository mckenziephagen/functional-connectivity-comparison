...
sys.path.append(os.path.dirname('/global/u1/m/mphagen/functional_connectivity_comparison/fc_comparison'))
...
for file in rest_scans: 
    ...
    os.makedirs(op.join(results_path, ses_string.split('_')[0],  f'{atlas_name}-{n_rois}'), exist_ok=True)
    ts.tofile(op.join(results_path, 
                       ses_string.split('_')[0], 
                       f'{atlas_name}-{n_rois}',
                       f'sub-{subject_id}_{ses_string}_task-Rest_atlas-{atlas_name}{n_rois}_timeseries.tsv'), 
                        sep = '/t')
